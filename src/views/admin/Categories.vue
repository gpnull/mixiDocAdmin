<template>
  <div>
    <p>1234</p>
    <p>{{user.data.email}}</p>
    <v-data-table :headers="headers" :items="categories" :search="search" class="elevation-1" :loading="loading"
                  ...
                  <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                    <v-row>
                      ...
                      <v-col cols="12" sm="6" md="6">
                        ...
                        <v-text-field :rules="imageRule" :disabled="loading" v-model="editedItem.image"
                          label="Category Image Url" prepend-icon="mdi-image" @click:prepend="$refs.uploader.click()">
                        </v-text-field>
                      </v-col>
                    </v-row>
                  </v-form>
